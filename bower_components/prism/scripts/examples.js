 ...
 */function loadLanguage(lang){// at first we need to fetch all dependencies for the main language
...
return Promise.all(dependencies).then(function(){// If the main language itself isn't already loaded, load it now
...
if(!Prism.languages[lang]){return new Promise(function(resolve){$u.script("components/prism-"+lang+".js",resolve)})}})}/**
 ...
 */function getDependenciesOfLanguage(lang){if(!components.languages[lang]||!components.languages[lang].require){return[]}return"array"===$u.type(components.languages[lang].require)?components.languages[lang].require:[components.languages[lang].require]}})();